...
La rete discreta di Hopfield è composta da singoli neuroni, modellati tramite
...
potrebbe uscire, poiché il livello di energia non può mai aumentare. Di solito,
...
La \emph{rete continua di Hopfield} rappresenta un cambio di paradigma: mentre
...
analogico}. Il valore d'uscita dipende da una funzione sigmoidale, ad esempio
...
mentre l'energia della reta sarà
...
TODO equazioni dell'energia 16

Dato che il rapporto ingresso-uscita è fissato da una sigmoide, non è garantito
che il valore d'uscita sia ad un valore nettamente basso o nettamente alto.
Esistono infatti valori in concomitanza con lo $0$ d'ingresso che possono
suscitare dubbi interpretativi, cioè il neurone potrebbe collocarsi ad un
valore intermedio fra valore alto e basso. 
...
stabile vero o falso che sia.

Individuata la forma quadratica associata al problema, per confronto si
ricavano i valori $T_{ij}$ ed $I_i$ che consentono la costruzione della rete.
...
$n$ neuroni. La quantità da minimizzare sarà $E^* = -\frac 1 2 \sum_s (V\cdot
V^s)^2$, con $V=V^s$ valore negativo di energia minimo: ciò accade poiché siamo
di fronte ad un prodotto scalare che massimizza la quantità all'interno della
sommatoria, con il risultato di minimizzare il valore dell'energia. Se $V$ è
casuale e $(-1 \leq V_i \leq +1)$, la $E^*$ deve essere circa nulla, con valore
minimo pari a $V=V^s$ tale che $E^* = -\frac 1 2 n^2$. Bisogna in pratica fissare i $V^s$ di stato stabile, dunque si ottiene

$$ E = -\frac 1 2 \sum_s (V^s V)^2 = \dots = -\frac 1 2 \sum_i \sum_j (\sum_s V_i^s V_j^s)V_i V_j,$$
